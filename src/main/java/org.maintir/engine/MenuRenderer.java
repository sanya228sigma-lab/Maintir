...
public class MenuRenderer {
    ...
    /** Цветная плашка без наведения (для рамок, фонов, курсора ввода). */
    public static void drawFlatRect(int screenW, int screenH, float x, float y, float w, float h, float r, float g, float b) {
    ...
    public static boolean drawColorButton(int screenW, int screenH, float mx, float my, float x, float y, float w, float h, float r, float g, float b, String label, boolean clicked) {
        boolean hovered = isHovered(mx, my, x, y, w, h);
        if (hovered) { r += 0.15f; g += 0.15f; b += 0.15f; }

        // Подсветка рамкой при наведении
        if (hovered) drawFlatRect(screenW, screenH, x - 2, y - 2, w + 4, h + 4, r + 0.2f, g + 0.2f, b + 0.2f);
